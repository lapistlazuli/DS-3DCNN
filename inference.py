...
if MODEL in ['SVM', 'SVM_grid', 'SGD', 'nearest']:
    ...
    model = medcam.inject(model, output_dir="attention_maps", save_maps=True, label=2,replace=True, layer="conv4")
 
    probabilities = test(model, img, hyperparams)
    prediction = np.argmax(probabilities, axis=-1)


gt = open_file('Datasets/IndianPines/Indian_pines_gt.mat')
gt = gt['indian_pines_gt']
gt = np.asarray(gt, dtype='int')

for ir, row in enumerate(gt):
